...
class Model:
    ...
    def access_BD(self, name, surname):
        ...
        try:
            ...
            self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controller.view)
            return
        except requests.exceptions.Timeout as t:
            self.controllerMain.create_dialog("El servidor no responde", self.controllerMain.controller.view)
            return
        data = r.json()
        length = 0
        values = None
    ...
    def get_instalaciones(self, fromAlerts, offset, limit, getLength): #este método accede a la BD y coge las instalaciones de dicho usuario desde offset y coge hasta limit elementos
        try:
            if offset is not None and limit is not None:
                r_offset = requests.get("http://localhost:8080/api/rest/user_access_log/"+ self.uuid+"?offset="+str(offset)+"&limit="+str(limit), headers={"x-hasura-admin-secret":"myadminsecretkey"})
            if getLength == True: r = requests.get("http://localhost:8080/api/rest/user_access_log/"+ self.uuid, headers={"x-hasura-admin-secret":"myadminsecretkey"}) # cogemos las instalaciones
        except requests.exceptions.ConnectionError as e:
            self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controllerInfo.view)
            return
        except requests.exceptions.Timeout as t:
            self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controllerInfo.view)
            return
        ...
        if data_offset["access_log"] == []: # si la lista está vacía y NO llamamos a la función desde get_alerts(), mostramos este diálogo
            if fromAlerts == False:
                self.controllerMain.create_dialog("Este usuario aún no ha visitado ninguna instalación", self.controllerMain.get_view_info())
                if getLength:
                    return None, 0
                else: return None
            if getLength:
        ...
        i = 0
        for key, value in data_offset.items(): # value es el array que contiene todos los diccionarios de cada key
            for i in range(len(value)):
                ...
                content.append(info)
        if getLength:
            return content, len(data["access_log"])
        else: return content

    def get_instalaciones_alertas(self, fromAlerts):
        try:
            r = requests.get("http://localhost:8080/api/rest/user_access_log/"+ self.uuid, headers={"x-hasura-admin-secret":"myadminsecretkey"})
        except requests.exceptions.ConnectionError as e:
            self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controllerInfo.view)
            return
        except requests.exceptions.Timeout as t:
        ...
        content = []
        i = 0
        for i in range(len(data["access_log"])):
            fecha_hora = data["access_log"][i]["timestamp"].split("T")
            fecha = fecha_hora[0]
            hora = fecha_hora[1].split(".")[0]
            info = {
                "Fecha": fecha, # incluye fecha y hora --> hay que separar (value[1] accede a la segunda hora, es decir, a la segunda instalación)
                "Hora": hora,
                "Temperatura": data["access_log"][i]["temperature"],
                "Nombre": data["access_log"][i]["facility"]["name"],
                "ID": data["access_log"][i]["facility"]["id"]
            }
            content.append(info)
        return content
    ...
    def get_alerts(self, withRange): # coger todas las instalaciones donde estuvo el usuario y coger todos los datos de cada una de las instalaciones y meterlos en un diccionario
        infoInstalaciones = self.get_instalaciones_alertas(True)
        if infoInstalaciones == None:
            self.controllerMain.create_dialog("Este usuario aún no ha vistado ninguna instalación, por lo tanto no ha recibido ninguna alerta", self.controllerMain.get_view_info())
            return None
        auxVacunado = ""
        i = 0
        listID = []
        ...
        for i in range(len(listID)): # ahora hay que hacer una request a la BD por cada uno de los ids y guardar todo en un json
            try:
                ...
                self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controllerAlerts.view)
                return
            except requests.exceptions.Timeout as t:
                self.controllerMain.create_dialog("No se puede conectar con el servidor", self.controllerMain.controllerAlerts.view)
                return
...
class ControllerInfo:
    ...
    def show_instalaciones(self, widget):
        ...
        self.controllerZ.get_instalaciones(0, 20, True)
...
class ControllerAlerts():
    ...
    def get_content(self, content):
        ...
        if len(self.content) % self.elementsPage == 0:
            self.pages -= 1
...
class ControllerInstalaciones():
    ...
    def get_content(self, content, length):
        self.content = sorted(content, key=lambda k: k['Fecha'], reverse = True) # lista ordenada desde la última instalación visitada
        self.pages = (int(length / self.elementsPage)) + 1 # número de páginas (se puede usar para indicar a partir de qué offset se empieza a mostrar, elementsPage * currentPage)
        if length % self.elementsPage == 0:
            self.pages -= 1

        self.view.hbox.pack_start(self.view.showPage, False, False, 0)
        self.leftValues = length
    ...
    def next_page(self, widget):
        if self.view.currentPage < self.pages:
            self.view.currentPage += 1
        self.leftValues -= self.elementsPage
        self.content = self.controllerZ.model.get_instalaciones(False, self.elementsPage * self.view.currentPage, self.leftValues+self.elementsPage, False)
        self.update_view_instalaciones()
    ...
    def previous_page(self, widget):
        if self.view.currentPage > 0:
            self.view.currentPage -= 1
        self.leftValues += self.elementsPage
        modulo = self.leftValues % self.elementsPage #cogemos el módulo. Pa sacar el límite correspondiente, cogemos el módulo y le sumamos la diferencia entre el
        # el número de elementos por página (20) y el propio modulo para obtener un múltiplo de 20 (la página anterior siempre va a tener 20 elementos)
        self.content = self.controllerZ.model.get_instalaciones(False, self.elementsPage*self.view.currentPage, (modulo + (self.elementsPage - modulo)) * (self.view.currentPage + 1), False)
        self.update_view_instalaciones()
...
class ControllerZ:
    ...
    def get_instalaciones(self, offset, limit, getLength):
        content = None
        fullLength = 0
        content, fullLength = self.model.get_instalaciones(False, offset, limit, getLength)
        if content is not None and fullLength != 0:
            self.controllerInstalaciones.get_content(content, fullLength)
